...
// ============ DATABASE POSTGRESQL ============
const pool = new Pool({
    connectionString: process.env.DATABASE_URL || 'postgresql://localhost/bot_whatsapp',
    max: 20,
...
async function createClientForUser(userId) {
    ...
    client.on('ready', async () => { 
        ...
        await db.run("UPDATE wa_sessions SET connected=1, last_update=NOW() WHERE user_id=$1", [userId]);
        console.log(`[User ${userId}] Conectado ao WhatsApp`);
    });
    ...
    client.on('message', async (msg) => {
        ...
        await db.run(
            "INSERT INTO messages (user_id, sender, msg, opt, created_date) VALUES ($1, $2, $3, $4, $5)",
            [userId, msg.from, msg.body, body, today]
        );
        ...
        const blocked = await db.get(
            "SELECT phone_number FROM blacklist WHERE user_id=$1 AND phone_number=$2",
            [userId, msg.from]
        );
        if (blocked) return;
        
        const kw = await db.all(
            "SELECT response FROM keywords WHERE user_id=$1 AND keyword LIKE $2",
            [userId, `%${body}%`]
        );
        
        if (kw?.length) {
            await msg.reply(kw[0].response);
            return;
        }
...
// Auto-reconnect sessões salvas ao iniciar
async function autoReconnectSavedSessions() {
    console.log('[Boot] Verificando sessoes salvas para auto-reconexao...');
...
app.get('/api/users', auth, adminAuth, async (req, res) => {
    try {
        ...
        const result = users.map(u => ({ 
            ...u, 
            wa_connected: connectedUsers.has(u.id),
            wa_status: clientStates[u.id] || { status: 'disconnected' }
        }));
        res.json(result);
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/users', auth, adminAuth, async (req, res) => {
    try {
        ...
        const result = await db.run(
            "INSERT INTO users (username, password_hash, role, created_at) VALUES ($1, $2, 'user', NOW()) RETURNING id",
            [username, bcrypt.hashSync(password, 10)]
        );
        
        // Criar sessão vazia
        await db.run(
            "INSERT INTO wa_sessions (user_id, connected) VALUES ($1, 0)",
            [result.rows[0].id]
        );
...
app.post('/api/admin/users/:id/reconnect', auth, adminAuth, async (req, res) => {
    try {
        ...
        if (clients[userId]) {
            await clients[userId].logout().catch(() => {});
...
app.post('/api/config', auth, async (req, res) => {
    try {
        ...
        if (cfg) {
            await db.run(
                "UPDATE configs SET menu_message=$1, default_reply=$2 WHERE user_id=$3",
                [menu_message, default_reply, userId]
            );
        } else {
            await db.run(
                "INSERT INTO configs (user_id, menu_message, default_reply) VALUES ($1, $2, $3)",
                [userId, menu_message, default_reply]
            );
        }
...
app.post('/api/options', auth, async (req, res) => {
    try {
        const result = await db.run(
            "INSERT INTO options (user_id, key_num, title, response) VALUES ($1, $2, $3, $4) RETURNING id",
            [req.session.user.id, req.body.key_num, req.body.title, req.body.response]
        );
        res.json({ success: true, id: result.rows[0].id });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.delete('/api/options/:id', auth, async (req, res) => {
    try {
        await db.run(
            "DELETE FROM options WHERE id=$1 AND user_id=$2",
...
app.post('/api/keywords', auth, async (req, res) => {
    try {
        const result = await db.run(
            "INSERT INTO keywords (user_id, keyword, response) VALUES ($1, $2, $3) RETURNING id",
            [req.session.user.id, req.body.keyword, req.body.response]
        );
        res.json({ success: true, id: result.rows[0].id });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.post('/api/blacklist', auth, async (req, res) => {
    try {
        const result = await db.run(
            "INSERT INTO blacklist (user_id, phone_number) VALUES ($1, $2) RETURNING id",
            [req.session.user.id, req.body.phone_number]
        );
        res.json({ success: true, id: result.rows[0].id });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.delete('/api/blacklist/:id', auth, async (req, res) => {
    try {
        await db.run(
            "DELETE FROM blacklist WHERE id=$1 AND user_id=$2",
            [req.params.id, req.session.user.id]
        );
        res.json({ success: true });
    } catch (err) {
...
app.get('/api/messages', auth, async (req, res) => {
    try {
        ...
        const messages = await db.all(
            "SELECT * FROM messages WHERE user_id=$1 AND created_date=$2 ORDER BY id DESC LIMIT 200",
            [userId, today]
        );
        res.json(messages);
    } catch (err) {
...
app.post('/api/pause', auth, async (req, res) => {
    try {
        await db.run("UPDATE users SET is_paused=1 WHERE id=$1", [req.session.user.id]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/resume', auth, async (req, res) => {
    try {
        await db.run("UPDATE users SET is_paused=0 WHERE id=$1", [req.session.user.id]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/admin/users/:id/pause', auth, adminAuth, async (req, res) => {
    try {
        await db.run("UPDATE users SET is_paused=1 WHERE id=$1", [req.params.id]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/admin/users/:id/resume', auth, adminAuth, async (req, res) => {
    try {
        await db.run("UPDATE users SET is_paused=0 WHERE id=$1", [req.params.id]);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get('/api/qr', auth, async (req, res) => {
    ...
    try {
        const url = await QRCode.toDataURL(qr, { width: 280, margin: 2 });
        res.json({ qr: url });
    } catch (e) {
        res.json({ qr: null });
    }
...
app.get('/api/whatsapp/status', auth, async (req, res) => {
    ...
    // Auto-reconnect se sessão existe e cliente não está inicializado
...
app.post('/api/whatsapp/init', auth, async (req, res) => {
    try {
        await createClientForUser(req.session.user.id);
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
    ...
    try {
        ...
        if (uid) {
            messages = await db.all(
                "SELECT m.*, u.username FROM messages m JOIN users u ON m.user_id=u.id WHERE m.user_id=$1 AND m.created_date=$2 ORDER BY m.id DESC LIMIT 500",
                [uid, today]
            );
        } else {
            messages = await db.all(
                "SELECT m.*, u.username FROM messages m JOIN users u ON m.user_id=u.id WHERE m.created_date=$1 ORDER BY m.id DESC LIMIT 500",
                [today]
            );
        }
...
app.post('/api/password', auth, async (req, res) => {
    try {
        ...
        await db.run(
            ...
            [bcrypt.hashSync(newpass, 10), req.session.user.id]
        );
...
function startServer(port = process.env.PORT || 3000) {
    app.listen(port, () => { 
        const host = isRender ? 'https://seu-bot.onrender.com' : `http://localhost:${port}`; 
        console.log('\n? Bot: ' + host + '/admin\n'); 
    }).on('error', (err) => {
